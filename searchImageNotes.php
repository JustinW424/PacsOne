<?php
//
// searchImageNotes.php
//
// Module for searching image notes information
//
...
print "<head>";
print "<title>$PRODUCT - ";
print pacsone_gettext("Image Notes Search Results:");
print "</title></head>";
print "<body>";
...
$error = searchImageNotes($username, $author, $subject, $compare, $date);
if ($error) {
...
function searchImageNotes($username, $author, $subject, $compare, $date)
{
    global $dbcon;
    ...
    $query = "SELECT * FROM imagenotes where ";
    $key = "";
    $value = "";
    $bindList = array();
    ...
    if (strlen($key) == 0) {
        $error = "<br>";
        $error .= pacsone_gettext("You must enter a criteria for <a href='search.php'>Search By Image Notes</a>");
        return $error;
    }
    $query .= $key;
    if (count($bindList))
    ...
    if ($result) {
        ...
        print "<p>";
        if ($count)
            displayNotes("imagenotes", $rows, $username, "imageNotes.php", 0, 1);
    }
    return $error;
}
