...
// liveMonitor.php
//
// Module for modifying entries in the Live Monitor Table
//
...
if (isset($result)) {       // display results
    if (empty($result)) {   // success
        header('Location: tools.php?page=' . urlencode(pacsone_gettext("Live Monitor")));
        exit();
    }
    else {                  // error
        print "<html>\n";
        print "<head><title>$PRODUCT - ";
        print pacsone_gettext("Live Monitor Table Error");
        print "</title></head>\n";
        print "<body>\n";
...
function deleteEntries($entry)
{
	...
	foreach ($entry as $uid) {
		$query = "delete from monitor where id=?";
        $bindList = array($uid);
	...
	if (!empty($errors)) {
        if (count($errors) == 1)
		    $result = pacsone_gettext("Error deleting the following Live Monitor table entry:");
        else
		    $result = pacsone_gettext("Error deleting the following Live Monitor table entries:");
        $result .= "<P>\n";
